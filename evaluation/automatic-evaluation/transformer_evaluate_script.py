...
from transformer_evaluation import *

parser = argparse.ArgumentParser(description='Transformer script to produce evaluation metrics')
...
transformer = TransformerModel(ntokens, emsize, nhead, nhid, nlayers, embedding, dropout)
...
if "csv" in evaluation_file_extension:
    if "eval_topX_recall" in args.evaluation_file: 
        metrics = calculate_evaluation_metrics(args.evaluation_file, voc, transformer, embedding, 10, 5, "¤",
